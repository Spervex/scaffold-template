...
> = {
  ...
  compression: {
    ...
    meta: {
      category: 'backend',
      label: 'Compression',
      hint: 'Gzip response compression',
  ...
  swagger: {
    ...
    meta: {
      ...
      hint: 'swagger-ui-express + swagger-jsdoc',
      defaultSelected: false,
    },
    files: () => [],
  },
  ...
  tailwindcss: {
    ...
    files: (options: CliOptions, baseDir: string) => {
      const isVite =
        options.frontendFramework === 'vite' || options.projectType === 'fullstack-vite';
      if (isVite) {
        return [
          {
            path: path.join(baseDir, 'tailwind.config.ts'),
            content: `import type { Config } from 'tailwindcss';\n\nexport default {\n  content: ['./index.html', './src/**/*.{js,ts,jsx,tsx}'],\n  theme: { extend: {} },\n  plugins: [],\n} satisfies Config;\n`,
          },
          {
            path: path.join(baseDir, 'postcss.config.js'),
            content: `export default {\n  plugins: {\n    tailwindcss: {},\n    autoprefixer: {},\n  },\n};\n`,
          },
          {
            path: path.join(baseDir, 'src', 'index.css'),
  ...
  'tanstack-query': {
    ...
    meta: {
      category: 'frontend',
      label: 'TanStack Query + Axios',
      hint: 'Server state & data fetching',
      defaultSelected: false,
    },
    files: () => [],
  },
  'react-router': {
    ...
    meta: {
      category: 'frontend',
      label: 'React Router',
      hint: 'Client-side routing (Vite only)',
      defaultSelected: false,
    },
    files: () => [],
  },
  shadcn: {
    ...
    meta: {
      category: 'frontend',
      label: 'shadcn/ui',
      hint: 'Component library (via CLI)',
      defaultSelected: false,
    },
    files: (options: CliOptions, baseDir: string) => [
      {
          ...
          JSON.stringify(
            {
              ...
              style: 'default',
              rsc: options.frontendFramework === 'nextjs',
              tsx: true,
              tailwind: {
                config: 'tailwind.config.ts',
                css: options.frontendFramework === 'nextjs' ? 'app/globals.css' : 'src/index.css',
                baseColor: 'slate',
                cssVariables: true,
              },
      ...
      {
        path: path.join(baseDir, 'src', 'lib', 'utils.ts'),
  ...
  motion: {
    ...
    meta: {
      category: 'frontend',
      label: 'Motion',
      hint: 'Animations (formerly Framer Motion)',
      defaultSelected: false,
    },
    files: () => [],
  },
  ...
  sentry: {
    ...
    meta: {
      category: 'infra',
      label: 'Sentry',
      hint: 'Error tracking & performance',
      defaultSelected: false,
    },
    files: () => [],
  },
  ...
  'socket-io': {
    ...
    meta: {
      category: 'infra',
      label: 'Socket.IO',
      hint: 'Real-time bidirectional communication',
      defaultSelected: false,
    },
    files: () => [],
  },
...
export const ALL_EXTRAS: ExtraPackage[] = Object.keys(EXTRA_CONFIG) as ExtraPackage[];

export function addDatabaseDeps(
  pkg: {
    dependencies: Record<string, string>;
    devDependencies: Record<string, string>;
  ...
  if (database === 'mongodb') {
    pkg.dependencies.mongoose = '^9.6.1';
  } else {
